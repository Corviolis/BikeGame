...
public partial class Player : CharacterBody3D
{
	...
	private void RotateBike(Vector2 direction, float delta) {
		// turns 180 faster than 90, will need to refactor
		// for clarification, this is because we are moving from point a to point b over a set time,
		//   rather than changing the angle by a set amount and interpolating between those values
		if (direction != Vector2.Zero) {
			float interpolatedAngle = Mathf.LerpAngle(Rotation.Y, direction.Angle(), _turnSpeed * delta);
